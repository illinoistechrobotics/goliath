...
void refresh_ui(packet_t * ctl, char * msg, int overflow){
  ...
  if(ctl->stickY != old_leftY){
    old_leftY = ctl->stickY;
    setCDKSliderValue(leftY, ctl->stickY);
    drawCDKSlider(leftY, true);
  }

  if(ctl->stickX != old_rightY){
    old_rightY = ctl->stickX;
    setCDKSliderValue(rightY, ctl->stickX);
    drawCDKSlider(rightY, true);
  }
}
